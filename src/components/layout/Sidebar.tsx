...
const Sidebar: React.FC = () => {
  ...
  return (
    <aside className={`${isSidebarCollapsed ? 'w-20' : 'w-64'} bg-theme-bg-alt border-r border-theme-border flex flex-col h-full overflow-hidden transition-all duration-300 relative`}>
      ...
      <div className={`px-4 py-2 ${isSidebarCollapsed ? 'flex flex-col items-center' : ''}`}>
        <button
          onClick={() => setIsSidebarCollapsed(!isSidebarCollapsed)}
          className={`flex items-center text-theme-text-muted hover:text-theme-text transition-colors text-sm font-medium ${
            isSidebarCollapsed ? 'justify-center w-10 h-10' : 'w-full px-4 py-2 space-x-3'
          }`}
